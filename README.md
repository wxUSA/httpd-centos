![rpmbuild](https://copr.fedorainfracloud.org/coprs/kremit/httpd24/package/httpd/status_image/last_build.png)

## About this repo

This is an RPM packaging fork of [apache/httpd](https://github.com/apache/httpd) for RHEL/CentOS/Rocky, built and distributed via [Fedora COPR](https://copr.fedorainfracloud.org/coprs/kremit/httpd24/).

### Branch layout

| Branch | Purpose |
|---|---|
| `2.4.x` *(default)* | Always the current packaging release; what COPR builds from |
| `build/N.N.NN` | Per-version packaging branch; `2.4.x` on this remote is force-pushed from here |
| `origin/2.4.x` on [apache/httpd](https://github.com/apache/httpd) | Clean upstream source with **no packaging changes** |

### Making a new release

```bash
./rpm/build.sh 2.4.67        # new upstream version, release 1
./rpm/build.sh 2.4.66 2      # re-release of existing version
```

The script creates `build/N.N.NN` from the upstream release tag, copies the packaging layer, commits, tags, and force-pushes to this branch. Note the script also uses `build/N.N.NN:2.4.x` to push to this GitHub branch.
